...
/*   Updated: 2021/11/15 18:17:40 by Clkuznie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    void )
{
    ...
    NEXT_TEST
    {
        std::cout << "Double ended queue with 100 random values find random one in the set\n\n";
    
        std::deque<int> deque(100);
    
        std::cout << "Default:\n";
        std::for_each(deque.begin(), deque.end(), intPrint);
        std::cout << "\n\n";

        std::cout << "Randomly filled\n";
        std::generate(deque.begin(), deque.end(), myRandom);
        std::for_each(deque.begin(), deque.end(), intPrint);
        std::cout << "\n\n";

        int i = std::rand() % 100;
        std::cout << "Find the number at index = " << i << "\n";
        easyfind< std::deque<int> >(deque, *(deque.begin() + i));

        i = std::rand() % 1000000;
        std::cout << "\nFind a random number maybe not in the set\n";
        std::cout << "Number -  " << i << "\n";
        easyfind< std::deque<int> >(deque, i);
